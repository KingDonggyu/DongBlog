...
이전 포스트까지는 대부분 레이블(label-y)이 존재하는 데이터에 대해 다루었다. 하지만 우리가 사용하는 대부분의 데이터는 레이블이 없다. 시스템을 학습시키기 위해 데이터를 마련하는 건 쉬울지 몰라도 이것들에 각각 결함과 정상을 판단하여 레이블링을 하는 것은 높은 cost를 요한다.
